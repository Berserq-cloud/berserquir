...
let evt = {};
try { evt = JSON.parse(readFileSync(0, "utf8")); } catch { /* no/invalid stdin */ }

const paths = [
  evt.tool_input?.file_path, evt.tool_input?.filePath,
  evt.file_path, evt.filePath, evt.path,
...
for (const p of paths) {
  ...
  if (existsSync(journal)) spawnSync(process.execPath, [journal, agent, tool, p], { encoding: "utf8" });
}
