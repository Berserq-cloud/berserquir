...
7. **Your proficiency** — per selected area: novice / competent / senior? **Mandatory — asked in every init, never inferred** (a codebase reflects its past authors, not today's operator). Honest answers calibrate mentorship (`core/protocols/mentorship.md`) — seeds `.berserqir/memory/human-profile.md`; behavior refines it over time
