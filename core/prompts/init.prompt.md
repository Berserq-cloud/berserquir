...
Then generate, in order, each after an OK: `PRD.md` (from `core/templates/prd.template.md`) → `SPECS.md` + `TESTS.md` skeletons → `memory-long.md` (constitution) → `memory-medium.json` (empty tracker) + `memory-short.md` (fresh) + `human-profile.md` (seeded) + `instincts.json` (empty, from `core/memory/templates/instincts.template.json` — the learning pipeline fills it) → `codemap.md` seed + `graph.json` (from `core/memory/templates/graph.template.json` — shape enforced by `core/memory/schemas/graph.schema.json`).
