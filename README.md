...
- **Guardrail hooks (deterministic, zero-LLM)** — git-safety (no push/force/reset without a human), cmd-safety (destructive commands — `rm -rf`, `DROP TABLE`, `terraform destroy`, publishing — need a literal human OK, at every tier), secret-scan, config-protection ("fix the code, not the ruler"), memory validation, commit quality. Plus **advisory hooks** that surface but never block: front-quality (anti-slop tells, a11y signals + color drift against your `DESIGN.md` — rules distilled from [Impeccable](https://github.com/pbakaus/impeccable), Apache 2.0), back-quality (console/print leftovers, swallowed errors, hardcoded endpoints, `any` accumulation) and stray-doc (root-level `NOTES.md` scatter).
