...
- **15 behavioral evals with anti-checks** — including the ones that catch over-ceremony, not just under-performance. `/run-evals` smoke-tests your installed harness.
...
- **Commands** — `/berserqir init | compress | learn | evolve | sprint | evals | review | checkpoint | status`, plus `npx berserqir doctor` for a deterministic health score of the installation itself. `/learn` extracts **instincts** (project-specific patterns, confidence-scored, evidence-backed) from the session journal and may propose one exemplary trajectory as an ICL demo (human-approved); `/evolve` clusters mature instincts into generated skills — eval-gated and human-approved.
